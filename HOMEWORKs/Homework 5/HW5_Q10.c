...
struct polygon* copy_polygon(struct polygon* PG)
{
	struct points **points = (int**)malloc(PG->n * sizeof(*points));
	if (*points == NULL)
	{
		printf("out of memory : Error in memory allocation\n");
		exit(EXIT_FAILURE);
	...
	{
		points[i] = new_Point(PG->points[i]->point_coordinates[0][0], PG->points[i]->point_coordinates[0][1]);
	}
	return new_polygon(PG->n, points);
}
...
int main()
{
	...
	struct polygon *PG_COPY = copy_polygon(PG);
	printf("\nCopied Polygon:\t\t");
	for (int i = 0; i < n; i++)
	{
		printf("\tP%d(%d,%d)\t", i + 1, PG_COPY->points[i]->point_coordinates[0][0], PG_COPY->points[i]->point_coordinates[0][1]);
	}
	for (int i = 0; i < n; i++)
	{
		PG_COPY->points[i] = new_Point(2 * i + 1, 2 * i + 1);
	}
	printf("\nChanged Copied Polygon:\t");
	for (int i = 0; i < n; i++)
	{
		printf("\tP%d(%d,%d)\t", i + 1, PG_COPY->points[i]->point_coordinates[0][0], PG_COPY->points[i]->point_coordinates[0][1]);
	}
